<?php
/* 
 Template Name: Home
 */
?>
<?php get_header('home'); ?>

<main>
    <div class="concept">
        <div class="container">
            <div class="concept-box">
                <div class="concept-images">
                    <div class="concept-big-img">
                        <?php if (get_field('zobrazhennya_1_h_b2')) : ?>
                            <img src="<?php the_field('zobrazhennya_1_h_b2'); ?>" alt="concept-img" loading="lazy">
                        <?php endif ?>
                    </div>
                    <div class="concept-sm-img">
                        <?php if (get_field('zobrazhennya_2_h_b2')) : ?>
                            <img src="<?php the_field('zobrazhennya_2_h_b2'); ?>" alt="concept-img" loading="lazy">
                        <?php endif ?>
                    </div>
                </div>
                <div class="concept-text">
                    <h2><?php the_field('zagolovok_h_b2'); ?></h2>
                    <p><?php the_field('opys_h_b2'); ?></p>
                    <a href="/choose-flat/">Обрати квартиру</a>
                </div>
            </div>
        </div>
    </div>

    <?php include 'components/genplan_main.php' ?>

    <?php if (get_field('plan_h_b6')) : ?>

        <!--        <div class="plan">
            <img src="<?php the_field('plan_h_b6'); ?>" alt="plan-img" loading="lazy">
        </div>-->
    <?php endif ?>

    <div class="project">
        <div class="project-inner">
            <div class="project-img">
                <?php if (get_field('zobrazhennya_h_b4')) : ?>
                    <img src="<?php the_field('zobrazhennya_h_b4'); ?>" alt="project-img" loading="lazy">
                <?php endif ?>
            </div>
            <div class="project-info">
                <div class="project-text">
                    <h3><?php the_field('zagolovok_h_b4'); ?></h3>
                    <p><?php the_field('opys_h_b4'); ?></p>
                </div>
            </div>
        </div>
        <div class="project-inner">
            <div class="project-img">
                <?php if (get_field('zobrazhennya_h_b5')) : ?>
                    <img src="<?php the_field('zobrazhennya_h_b5'); ?>" alt="project-img" loading="lazy">
                <?php endif ?>
            </div>
            <div class="project-info">
                <div class="project-text">
                    <?php the_field('opys_h_b5'); ?>
                    <a href="/about/">Дізнатись більше</a>
                </div>
            </div>
        </div>
    </div>

    <div class="advantages">
        <div class="container">
            <h2><?php the_field('zagolovok_h_b3'); ?></h2>
            <div class="advantages-slider">
                <div class="swiper advantages-swiper">
                    <div class="swiper-wrapper">
                        <?php if (have_rows('perevagy_h_b3')) : ?>
                            <?php while (have_rows('perevagy_h_b3')) : the_row(); ?>
                                <div class="swiper-slide">
                                    <?php if (get_sub_field('zobrazhennya_perevagy_h_b3')) : ?>
                                        <img src="<?php the_sub_field('zobrazhennya_perevagy_h_b3'); ?>" alt="icon" loading="lazy">
                                    <?php endif ?>
                                    <h3><?php the_sub_field('nazva_perevagy_h_b3'); ?></h3>
                                    <p><?php the_sub_field('opys_perevagy_h_b3'); ?></p>
                                </div>
                            <?php endwhile; ?>
                        <?php else : ?>
                            <?php // No rows found 
                            ?>
                        <?php endif; ?>
                    </div>
                    <div class="swiper-nav">
                        <div class="swiper-scrollbar"></div>
                        <div class="swiper-navigation">
                            <div class="swiper-button-next"></div>
                            <div class="swiper-button-prev"></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="choose-flat">
        <div class="container">
            <div class="choose-flat-inner">
                <div class="choose-flat-top">
                    <div class="choose-flat-title">
                        <h2>Обрати квартиру</h2>
                    </div>
                    <div class="choose-flat-tabs">
                        <ul>
                            <li class="tab-active" rel="one">1-кімнатні</li>
                            <li rel="two">2-кімнатні</li>
                            <li rel="all" class="tab_last">всi планування</li>
                        </ul>
                    </div>
                </div>
                <div class="tab-container">
                    <div id="one" class="tab-content">
                        <div class="flat-slider">
                            <div class="swiper flat-swiper">
                                <div class="swiper-wrapper">
                                    <?php
                                    $args = array(
                                        'post_type' => 'apartment',
                                        'posts_per_page' => -1,
                                        'meta_query' => array(
                                            array(
                                                'key'   => 'typ_kvartyry',
                                                'value' => '2',
                                            )
                                        ),

                                    );
                                    $apartment = new WP_Query($args);

                                    if ($apartment->have_posts()) {
                                        while ($apartment->have_posts()) {
                                            $apartment->the_post(); ?>
                                            <div class="swiper-slide">
                                                <div class="flat-details">
                                                    <a href="<?php the_permalink(); ?>">
                                                        <div class="flat-img">
                                                            <?php echo get_the_post_thumbnail($page->ID, 'full', array('loading' => 'lazy')); ?>
                                                        </div>
                                                        <div class="flat-descr">
                                                            <div class="icon">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="35" height="68" viewBox="0 0 35 68" fill="none">
                                                                    <path d="M33.6261 0C33.8624 0.0486892 34.1047 0.0912923 34.341 0.139981C34.9044 0.261705 35.0923 0.492978 34.959 1.05899C34.7772 1.8015 34.5652 2.53793 34.2865 3.25001C33.3353 5.72707 32.2932 8.16761 30.9724 10.4743C29.8576 12.434 28.446 14.1807 27.004 15.9031C25.2531 17.9846 23.3446 19.9078 21.2059 21.5876C18.8127 23.4621 16.25 25.0628 13.5539 26.4504C11.0759 27.7346 8.50097 28.7875 5.82912 29.597C5.01727 29.8404 4.54469 30.3516 4.37505 31.2159C4.03577 32.9261 3.59955 34.6241 3.26633 36.3343C2.99975 37.7037 2.79375 39.0914 2.63017 40.479C2.44841 42.0127 2.32724 43.5586 2.21213 45.1045C2.15154 45.9444 2.10913 46.7842 2.13942 47.6241C2.20607 49.6752 2.27877 51.7323 2.42418 53.7772C2.70287 57.715 3.35721 61.604 4.10847 65.4748C4.24782 66.1991 4.39929 66.9172 4.5447 67.6415C4.55681 67.6963 4.56287 67.7571 4.57499 67.8484C4.28417 67.8728 3.99942 67.9032 3.71466 67.9275C3.42385 67.9519 3.13304 67.9519 2.84828 67.9945C2.67258 68.0188 2.61805 67.9641 2.5817 67.7997C2.24242 66.2964 1.85467 64.8053 1.56385 63.296C1.19428 61.4093 0.867111 59.5104 0.588414 57.6054C0.382421 56.1995 0.261249 54.7815 0.17037 53.3634C0.0673735 51.781 0.00678801 50.1986 0.000729386 48.6101C-0.0113879 46.1878 0.127959 43.7777 0.382421 41.3676C0.891345 36.5595 1.81831 31.8367 3.2845 27.2355C4.8658 22.2631 7.13779 17.6255 10.1792 13.3834C12.9662 9.4944 16.4923 6.43915 20.6243 4.06555C23.6354 2.33708 26.8283 1.08942 30.2151 0.340824C30.8876 0.194757 31.5783 0.139981 32.2568 0.0426031C32.3295 0.0304308 32.3962 0.0121723 32.4689 0C32.8566 0 33.2383 0 33.6261 0ZM32.0387 3.14654C32.0508 2.56227 31.8691 2.36751 31.4692 2.36751C31.3238 2.36751 31.1723 2.36751 31.0269 2.41012C30.6271 2.51967 30.2272 2.62922 29.8334 2.76311C25.4651 4.20553 21.4725 6.36611 17.7888 9.10488C16.5953 9.99346 15.4078 10.8881 14.4021 11.9897C10.67 16.0735 7.86482 20.7294 6.023 25.9635C5.95635 26.1522 5.91394 26.353 5.88365 26.5478C5.81701 27.0286 6.08358 27.2964 6.56222 27.2173C6.78638 27.1808 7.01055 27.1138 7.22261 27.0286C10.9002 25.574 14.3415 23.6812 17.5768 21.4111C19.0309 20.3886 20.4668 19.3418 21.7209 18.082C24.8047 14.978 27.4584 11.5393 29.7001 7.7781C30.4271 6.56087 31.0754 5.29495 31.7297 4.0412C31.8994 3.71864 31.9721 3.34738 32.0387 3.15871V3.14654Z" fill="#123E3C" />
                                                                </svg>
                                                            </div>
                                                            <div class="text">
                                                                <h6><?php the_title(); ?></h6>
                                                                <p>Загальна площа: <?php the_field('area'); ?> кв.м</p>
                                                                <p>Житлова площа: <?php the_field('zhytlova_ploshha'); ?> кв.м</p>
                                                            </div>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                    <?php
                                        }
                                        /* Restore original Post Data */
                                        wp_reset_postdata();
                                    }
                                    ?>
                                </div>
                                <div class="swiper-nav">
                                    <div class="swiper-scrollbar"></div>
                                    <div class="swiper-navigation">
                                        <div class="swiper-button-next"></div>
                                        <div class="swiper-button-prev"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div id="two" class="tab-content">
                        <div class="flat-slider">
                            <div class="swiper flat-swiper">
                                <div class="swiper-wrapper">
                                    <?php
                                    $args = array(
                                        'post_type' => 'apartment',
                                        'posts_per_page' => -1,
                                        'meta_query' => array(
                                            array(
                                                'key'   => 'typ_kvartyry',
                                                'value' => '3',
                                            )
                                        ),

                                    );
                                    $apartment = new WP_Query($args);

                                    if ($apartment->have_posts()) {
                                        while ($apartment->have_posts()) {
                                            $apartment->the_post(); ?>
                                            <div class="swiper-slide">
                                                <div class="flat-details">
                                                    <a href="<?php the_permalink(); ?>">
                                                        <div class="flat-img">
                                                            <?php echo get_the_post_thumbnail($page->ID, 'full', array('loading' => 'lazy')); ?>
                                                        </div>
                                                        <div class="flat-descr">
                                                            <div class="icon">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="35" height="68" viewBox="0 0 35 68" fill="none">
                                                                    <path d="M33.6261 0C33.8624 0.0486892 34.1047 0.0912923 34.341 0.139981C34.9044 0.261705 35.0923 0.492978 34.959 1.05899C34.7772 1.8015 34.5652 2.53793 34.2865 3.25001C33.3353 5.72707 32.2932 8.16761 30.9724 10.4743C29.8576 12.434 28.446 14.1807 27.004 15.9031C25.2531 17.9846 23.3446 19.9078 21.2059 21.5876C18.8127 23.4621 16.25 25.0628 13.5539 26.4504C11.0759 27.7346 8.50097 28.7875 5.82912 29.597C5.01727 29.8404 4.54469 30.3516 4.37505 31.2159C4.03577 32.9261 3.59955 34.6241 3.26633 36.3343C2.99975 37.7037 2.79375 39.0914 2.63017 40.479C2.44841 42.0127 2.32724 43.5586 2.21213 45.1045C2.15154 45.9444 2.10913 46.7842 2.13942 47.6241C2.20607 49.6752 2.27877 51.7323 2.42418 53.7772C2.70287 57.715 3.35721 61.604 4.10847 65.4748C4.24782 66.1991 4.39929 66.9172 4.5447 67.6415C4.55681 67.6963 4.56287 67.7571 4.57499 67.8484C4.28417 67.8728 3.99942 67.9032 3.71466 67.9275C3.42385 67.9519 3.13304 67.9519 2.84828 67.9945C2.67258 68.0188 2.61805 67.9641 2.5817 67.7997C2.24242 66.2964 1.85467 64.8053 1.56385 63.296C1.19428 61.4093 0.867111 59.5104 0.588414 57.6054C0.382421 56.1995 0.261249 54.7815 0.17037 53.3634C0.0673735 51.781 0.00678801 50.1986 0.000729386 48.6101C-0.0113879 46.1878 0.127959 43.7777 0.382421 41.3676C0.891345 36.5595 1.81831 31.8367 3.2845 27.2355C4.8658 22.2631 7.13779 17.6255 10.1792 13.3834C12.9662 9.4944 16.4923 6.43915 20.6243 4.06555C23.6354 2.33708 26.8283 1.08942 30.2151 0.340824C30.8876 0.194757 31.5783 0.139981 32.2568 0.0426031C32.3295 0.0304308 32.3962 0.0121723 32.4689 0C32.8566 0 33.2383 0 33.6261 0ZM32.0387 3.14654C32.0508 2.56227 31.8691 2.36751 31.4692 2.36751C31.3238 2.36751 31.1723 2.36751 31.0269 2.41012C30.6271 2.51967 30.2272 2.62922 29.8334 2.76311C25.4651 4.20553 21.4725 6.36611 17.7888 9.10488C16.5953 9.99346 15.4078 10.8881 14.4021 11.9897C10.67 16.0735 7.86482 20.7294 6.023 25.9635C5.95635 26.1522 5.91394 26.353 5.88365 26.5478C5.81701 27.0286 6.08358 27.2964 6.56222 27.2173C6.78638 27.1808 7.01055 27.1138 7.22261 27.0286C10.9002 25.574 14.3415 23.6812 17.5768 21.4111C19.0309 20.3886 20.4668 19.3418 21.7209 18.082C24.8047 14.978 27.4584 11.5393 29.7001 7.7781C30.4271 6.56087 31.0754 5.29495 31.7297 4.0412C31.8994 3.71864 31.9721 3.34738 32.0387 3.15871V3.14654Z" fill="#123E3C" />
                                                                </svg>
                                                            </div>
                                                            <div class="text">
                                                                <h6><?php the_title(); ?></h6>
                                                                <p>Загальна площа: <?php the_field('area'); ?> кв.м</p>
                                                                <p>Житлова площа: <?php the_field('zhytlova_ploshha'); ?> кв.м</p>
                                                            </div>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                    <?php
                                        }
                                        /* Restore original Post Data */
                                        wp_reset_postdata();
                                    }
                                    ?>
                                </div>
                                <div class="swiper-nav">
                                    <div class="swiper-scrollbar"></div>
                                    <div class="swiper-navigation">
                                        <div class="swiper-button-next"></div>
                                        <div class="swiper-button-prev"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div id="all" class="tab-content">
                        <div class="flat-slider">
                            <div class="swiper flat-swiper">
                                <div class="swiper-wrapper">
                                    <?php
                                    $args = array(
                                        'post_type' => 'apartment',
                                        'posts_per_page' => -1,

                                    );
                                    $apartment = new WP_Query($args);

                                    if ($apartment->have_posts()) {
                                        while ($apartment->have_posts()) {
                                            $apartment->the_post(); ?>
                                            <div class="swiper-slide">
                                                <div class="flat-details">
                                                    <a href="<?php the_permalink(); ?>">
                                                        <div class="flat-img">
                                                            <?php echo get_the_post_thumbnail($page->ID, 'full', array('loading' => 'lazy')); ?>
                                                        </div>
                                                        <div class="flat-descr">
                                                            <div class="icon">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="35" height="68" viewBox="0 0 35 68" fill="none">
                                                                    <path d="M33.6261 0C33.8624 0.0486892 34.1047 0.0912923 34.341 0.139981C34.9044 0.261705 35.0923 0.492978 34.959 1.05899C34.7772 1.8015 34.5652 2.53793 34.2865 3.25001C33.3353 5.72707 32.2932 8.16761 30.9724 10.4743C29.8576 12.434 28.446 14.1807 27.004 15.9031C25.2531 17.9846 23.3446 19.9078 21.2059 21.5876C18.8127 23.4621 16.25 25.0628 13.5539 26.4504C11.0759 27.7346 8.50097 28.7875 5.82912 29.597C5.01727 29.8404 4.54469 30.3516 4.37505 31.2159C4.03577 32.9261 3.59955 34.6241 3.26633 36.3343C2.99975 37.7037 2.79375 39.0914 2.63017 40.479C2.44841 42.0127 2.32724 43.5586 2.21213 45.1045C2.15154 45.9444 2.10913 46.7842 2.13942 47.6241C2.20607 49.6752 2.27877 51.7323 2.42418 53.7772C2.70287 57.715 3.35721 61.604 4.10847 65.4748C4.24782 66.1991 4.39929 66.9172 4.5447 67.6415C4.55681 67.6963 4.56287 67.7571 4.57499 67.8484C4.28417 67.8728 3.99942 67.9032 3.71466 67.9275C3.42385 67.9519 3.13304 67.9519 2.84828 67.9945C2.67258 68.0188 2.61805 67.9641 2.5817 67.7997C2.24242 66.2964 1.85467 64.8053 1.56385 63.296C1.19428 61.4093 0.867111 59.5104 0.588414 57.6054C0.382421 56.1995 0.261249 54.7815 0.17037 53.3634C0.0673735 51.781 0.00678801 50.1986 0.000729386 48.6101C-0.0113879 46.1878 0.127959 43.7777 0.382421 41.3676C0.891345 36.5595 1.81831 31.8367 3.2845 27.2355C4.8658 22.2631 7.13779 17.6255 10.1792 13.3834C12.9662 9.4944 16.4923 6.43915 20.6243 4.06555C23.6354 2.33708 26.8283 1.08942 30.2151 0.340824C30.8876 0.194757 31.5783 0.139981 32.2568 0.0426031C32.3295 0.0304308 32.3962 0.0121723 32.4689 0C32.8566 0 33.2383 0 33.6261 0ZM32.0387 3.14654C32.0508 2.56227 31.8691 2.36751 31.4692 2.36751C31.3238 2.36751 31.1723 2.36751 31.0269 2.41012C30.6271 2.51967 30.2272 2.62922 29.8334 2.76311C25.4651 4.20553 21.4725 6.36611 17.7888 9.10488C16.5953 9.99346 15.4078 10.8881 14.4021 11.9897C10.67 16.0735 7.86482 20.7294 6.023 25.9635C5.95635 26.1522 5.91394 26.353 5.88365 26.5478C5.81701 27.0286 6.08358 27.2964 6.56222 27.2173C6.78638 27.1808 7.01055 27.1138 7.22261 27.0286C10.9002 25.574 14.3415 23.6812 17.5768 21.4111C19.0309 20.3886 20.4668 19.3418 21.7209 18.082C24.8047 14.978 27.4584 11.5393 29.7001 7.7781C30.4271 6.56087 31.0754 5.29495 31.7297 4.0412C31.8994 3.71864 31.9721 3.34738 32.0387 3.15871V3.14654Z" fill="#123E3C" />
                                                                </svg>
                                                            </div>
                                                            <div class="text">
                                                                <h6><?php the_title(); ?></h6>
                                                                <p>Загальна площа: <?php the_field('area'); ?> кв.м</p>
                                                                <p>Житлова площа: <?php the_field('zhytlova_ploshha'); ?> кв.м</p>
                                                            </div>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                    <?php
                                        }
                                        /* Restore original Post Data */
                                        wp_reset_postdata();
                                    }
                                    ?>
                                </div>
                                <div class="swiper-nav">
                                    <div class="swiper-scrollbar"></div>
                                    <div class="swiper-navigation">
                                        <div class="swiper-button-next"></div>
                                        <div class="swiper-button-prev"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!--     <div class="visit">
        <div class="visit-inner">
            <div class="visit-img">
                <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img5.jpg" alt="visit-img" loading="lazy">
            </div>
            <div class="visit-info">
                <div class="visit-form">
                    <h4>записатись на вiзит</h4>
                    <?php echo do_shortcode('[contact-form-7 id="fab2484" title="Form"]'); ?>
                </div>
            </div>
        </div>
    </div> -->


    <div class="gallery" id="gallery-home"> <!-- data-aos -->
        <div class="container gallery__container">

            <h2 id="g-title" class="gallery__title"><?php the_field('zagolovok_h_b7'); ?></h2>

            <div class="gallery__grid">

                <?php if (get_field('zobrazhennya_1_h_b7')) : ?>
                    <div class="gallery__item _1" id="gh-1">
                        <img src="<?php the_field('zobrazhennya_1_h_b7'); ?>" alt="gallery-img" loading="lazy">
                    </div>
                <?php endif ?>

                <?php if (get_field('zobrazhennya_2_h_b7')) : ?>
                    <div class="gallery__item _2" id="gh-2">
                        <div class="img">
                            <img src="<?php the_field('zobrazhennya_2_h_b7'); ?>" alt="gallery-img" loading="lazy">
                        </div>
                        <div class="gallery-main">
                            <h2><?php the_field('zagolovok_h_b7'); ?></h2>
                            <p><?php the_field('subtitle_b7'); ?></p>
                            <a href="<?= site_url('gallery') ?>">Дивитись галерею</a>
                        </div>
                    </div>
                <?php endif ?>

                <?php if (get_field('zobrazhennya_3_h_b7')) : ?>
                    <div class="gallery__item _3" id="gh-3">
                        <img src="<?php the_field('zobrazhennya_3_h_b7'); ?>" alt="gallery-img" loading="lazy">
                    </div>
                <?php endif ?>

                <?php if (get_field('zobrazhennya_4_h_b7')) : ?>
                    <div class="gallery__item _4" id="gh-4">
                        <img src="<?php the_field('zobrazhennya_4_h_b7'); ?>" alt="gallery-img" loading="lazy">
                    </div>
                <?php endif ?>

                <?php if (get_field('zobrazhennya_5_h_b7')) : ?>
                    <div class="gallery__item _5" id="gh-5">
                        <img src="<?php the_field('zobrazhennya_5_h_b7'); ?>" alt="gallery-img" loading="lazy">
                    </div>
                <?php endif ?>

            </div>
        </div>
    </div>

    <!-- <div class="gallery">
        <div class="container">
            <h2 data-aos="fade" class="title-mob"><?php the_field('zagolovok_h_b7'); ?></h2>
            <div class="gallery-inner">
                <div class="gallery-row">
                    <div class="gallery-left">
                        <h2 data-aos="fade"><?php the_field('zagolovok_h_b7'); ?></h2>
                        <?php if (get_field('zobrazhennya_1_h_b7')) : ?>
                            <img data-aos="fade" src="<?php the_field('zobrazhennya_1_h_b7'); ?>" alt="gallery-img" loading="lazy">
                        <?php endif ?>
                    </div>
                    <div class="gallery-right gallery-big-img">
                        <?php if (get_field('zobrazhennya_2_h_b7')) : ?>
                            <img data-aos="fade" src="<?php the_field('zobrazhennya_2_h_b7'); ?>" alt="gallery-img" loading="lazy">
                        <?php endif ?>
                    </div>
                </div>

                <div class="gallery-row">
                    <div class="gallery-left">
                        <?php if (get_field('zobrazhennya_3_h_b7')) : ?>
                            <img data-aos="fade" src="<?php the_field('zobrazhennya_3_h_b7'); ?>" alt="gallery-img" loading="lazy">
                        <?php endif ?>
                    </div>
                    <div class="gallery-right gallery-sm-img">
                        <?php if (get_field('zobrazhennya_4_h_b7')) : ?>
                            <img data-aos="fade" src="<?php the_field('zobrazhennya_4_h_b7'); ?>" alt="gallery-img" loading="lazy">
                        <?php endif ?>
                    </div>
                </div>

                <a href="/gallery/">Дивитись галерею</a>
            </div>
        </div>
    </div> -->


    <div class="progress">
        <div class="container">
            <div class="progress-inner">
                <div class="progress-title">
                    <h3>Хід будівництва</h3>
                    <a href="/type/hid-budivnycztva/">Дізнатися більше</a>
                </div>
                <div class="progress-slider">
                    <div class="swiper progress-swiper">
                        <div class="swiper-wrapper">
                            <?php
                            $args = array(
                                'post_type' => 'news',
                                'posts_per_page' => 5,
                                'tax_query' => array(
                                    array(
                                        'taxonomy' => 'types',
                                        'field' => 'id',
                                        'terms' => 6,
                                    ),
                                ),
                            );

                            $query = new WP_Query($args);

                            if ($query->have_posts()) :
                                while ($query->have_posts()) : $query->the_post(); ?>
                                    <div class="swiper-slide">
                                        <div class="card-details">
                                            <a href="<?php the_permalink(); ?>">
                                                <div class="card-img">
                                                    <?php echo get_the_post_thumbnail($page->ID, 'full', array('loading' => 'lazy')); ?>
                                                </div>
                                                <div class="card-info">
                                                    <div class="icon">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                                                            <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C" />
                                                        </svg>
                                                    </div>
                                                    <div class="text">
                                                        <p><?php the_title(); ?></p>
                                                    </div>
                                                </div>
                                            </a>
                                        </div>
                                    </div>
                            <?php
                                endwhile;
                                wp_reset_postdata();
                            else :
                                echo 'Нет постов для отображения.';
                            endif;
                            ?>

                            <!-- <div class="swiper-slide">
                    <div class="card-details">
                      <a href="#">
                        <div class="card-img">
                          <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img13.jpg" alt="card-img" loading="lazy">
                        </div>
                        <div class="card-info">
                          <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                              <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C"/>
                            </svg>
                          </div>
                          <div class="text">
                            <p>Хід будівництва 1.09.2023</p>
                          </div>
                        </div>
                      </a>
                    </div>
                  </div>
                  <div class="swiper-slide">
                    <div class="card-details">
                      <a href="#">
                        <div class="card-img">
                          <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img13.jpg" alt="card-img" loading="lazy">
                        </div>
                        <div class="card-info">
                          <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                              <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C"/>
                            </svg>
                          </div>
                          <div class="text">
                            <p>Хід будівництва 1.10.2023</p>
                          </div>
                        </div>
                      </a>
                    </div>
                  </div>
                  <div class="swiper-slide">
                    <div class="card-details">
                      <a href="#">
                        <div class="card-img">
                          <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img13.jpg" alt="card-img" loading="lazy">
                        </div>
                        <div class="card-info">
                          <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                              <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C"/>
                            </svg>
                          </div>
                          <div class="text">
                            <p>Хід будівництва 1.11.2023</p>
                          </div>
                        </div>
                      </a>
                    </div>
                  </div>
                  <div class="swiper-slide">
                    <div class="card-details">
                      <a href="#">
                        <div class="card-img">
                          <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img13.jpg" alt="card-img" loading="lazy">
                        </div>
                        <div class="card-info">
                          <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                              <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C"/>
                            </svg>
                          </div>
                          <div class="text">
                            <p>Хід будівництва 1.12.2023</p>
                          </div>
                        </div>
                      </a>
                    </div>
                  </div> -->
                            <!-- <div class="swiper-slide">
                    <div class="card-details">
                      <a href="#">
                        <div class="card-img">
                          <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img13.jpg" alt="card-img" loading="lazy">
                        </div>
                        <div class="card-info">
                          <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                              <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C"/>
                            </svg>
                          </div>
                          <div class="text">
                            <p>Хід будівництва 1.09.2023</p>
                          </div>
                        </div>
                      </a>
                    </div>
                  </div>
                  <div class="swiper-slide">
                    <div class="card-details">
                      <a href="#">
                        <div class="card-img">
                          <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img13.jpg" alt="card-img" loading="lazy">
                        </div>
                        <div class="card-info">
                          <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                              <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C"/>
                            </svg>
                          </div>
                          <div class="text">
                            <p>Хід будівництва 1.10.2023</p>
                          </div>
                        </div>
                      </a>
                    </div>
                  </div>
                  <div class="swiper-slide">
                    <div class="card-details">
                      <a href="#">
                        <div class="card-img">
                          <img src="<?php echo get_template_directory_uri(); ?>/img/content/content-img13.jpg" alt="card-img" loading="lazy">
                        </div>
                        <div class="card-info">
                          <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="24" viewBox="0 0 12 24" fill="none">
                              <path d="M11.5289 0C11.61 0.0171844 11.693 0.0322208 11.7741 0.0494052C11.9672 0.0923663 12.0316 0.173992 11.9859 0.373761C11.9236 0.635824 11.8509 0.895738 11.7554 1.14706C11.4292 2.02132 11.0719 2.88269 10.6191 3.6968C10.2369 4.38847 9.7529 5.00496 9.25852 5.61286C8.65819 6.3475 8.00386 7.02628 7.2706 7.61915C6.45009 8.28075 5.57141 8.84568 4.64704 9.33544C3.79745 9.78868 2.91462 10.1603 1.99856 10.446C1.72021 10.5319 1.55818 10.7123 1.50002 11.0174C1.38369 11.621 1.23413 12.2203 1.11988 12.8239C1.02848 13.3072 0.957859 13.7969 0.901773 14.2867C0.839456 14.828 0.797911 15.3736 0.758443 15.9192C0.73767 16.2157 0.72313 16.5121 0.733516 16.8085C0.756366 17.5324 0.781293 18.2585 0.831147 18.9802C0.9267 20.37 1.15104 21.7426 1.40862 23.1088C1.4564 23.3644 1.50833 23.6178 1.55818 23.8735C1.56234 23.8928 1.56441 23.9143 1.56857 23.9465C1.46886 23.9551 1.37123 23.9658 1.2736 23.9744C1.17389 23.983 1.07418 23.983 0.976554 23.9981C0.916314 24.0066 0.897618 23.9873 0.885155 23.9293C0.768829 23.3987 0.635886 22.8725 0.536178 22.3398C0.409466 21.6739 0.297295 21.0037 0.201742 20.3313C0.131116 19.8351 0.0895712 19.3346 0.0584126 18.8341C0.0230995 18.2756 0.00232732 17.7171 0.000250075 17.1565C-0.00390441 16.3016 0.0438716 15.4509 0.131116 14.6003C0.305604 12.9034 0.623422 11.2365 1.12611 9.61254C1.66827 7.85758 2.44724 6.22076 3.49002 4.72357C4.44555 3.35096 5.6545 2.27264 7.07118 1.4349C8.10357 0.824853 9.19828 0.384502 10.3595 0.120291C10.59 0.0687377 10.8268 0.0494052 11.0595 0.0150364C11.0844 0.0107403 11.1073 0.00429611 11.1322 0C11.2651 0 11.396 0 11.5289 0ZM10.9847 1.11054C10.9889 0.904331 10.9265 0.835593 10.7894 0.835593C10.7396 0.835593 10.6877 0.835593 10.6378 0.850629C10.5007 0.889294 10.3636 0.927959 10.2286 0.975216C8.7309 1.4843 7.36199 2.24686 6.09903 3.21349C5.68982 3.5271 5.28268 3.84287 4.93785 4.23167C3.65827 5.67301 2.69651 7.31627 2.06503 9.1636C2.04218 9.23019 2.02764 9.30107 2.01725 9.36981C1.9944 9.53951 2.0858 9.63402 2.2499 9.6061C2.32676 9.59321 2.40362 9.56958 2.47632 9.53951C3.73721 9.02612 4.91708 8.35808 6.02633 7.55685C6.52487 7.19598 7.01717 6.82651 7.44716 6.38187C8.50448 5.28636 9.41431 4.07271 10.1829 2.74521C10.4322 2.3156 10.6544 1.86881 10.8788 1.42631C10.9369 1.31246 10.9619 1.18143 10.9847 1.11484V1.11054Z" fill="#123E3C"/>
                            </svg>
                          </div>
                          <div class="text">
                            <p>Хід будівництва 1.11.2023</p>
                          </div>
                        </div>
                      </a>
                    </div>
                  </div> -->





                        </div>
                        <div class="swiper-nav">
                            <div class="swiper-scrollbar progress-scrollbar"></div>
                            <div class="swiper-navigation">
                                <div class="swiper-button-next pr-button-next"></div>
                                <div class="swiper-button-prev pr-button-prev"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="mob-link">
                    <a href="/type/hid-budivnycztva/">Дізнатися більше</a>
                </div>
            </div>
        </div>
    </div>
    <?php include 'components/complex.php' ?>
</main>

<?php get_footer();
